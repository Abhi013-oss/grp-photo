        ...
        <ScrollRevealStagger staggerDelay={0.12}>
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-8 mb-14">
            {servicesData.map((service) => (
              <ScrollRevealItem key={service.id} variant="fade-up" duration={0.7}>
                <Link
                  href={`/services#${service.slug}`}
                  className="group flex flex-col bg-[#FAF8F5] rounded-lg overflow-hidden border border-[#2C2523]/10 shadow-subtle hover:shadow-editorial transition-all duration-500 hover:-translate-y-1 h-full"
                >
                  {/* Image Frame */}
