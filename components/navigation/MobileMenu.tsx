    ...
    <AnimatePresence>
      {isOpen && (
        <motion.div
          ...
          className="fixed inset-0 z-50 flex flex-col justify-between bg-[#FAF8F5]/98 backdrop-blur-xl px-6 py-8 md:hidden text-espresso"
        >
          {/* Top Bar inside Menu */}
          <div className="flex items-center justify-between border-b border-[#2C2523]/10 pb-6">
            <div>
            ...
            <button
              ...
              className="p-3 rounded-full border border-[#2C2523]/15 text-espresso-primary hover:bg-peach-soft transition-colors"
            >
              <X className="w-5 h-5" />
            </button>
          </div>
          ...
          {/* Navigation Links */}
          <nav className="flex flex-col items-center gap-6 my-auto py-6 text-center" aria-label="Mobile primary navigation">
            {siteConfig.navItems.map((item, index) => (
              <Link
                key={item.label}
                href={item.href}
                onClick={onClose}
                className="group flex flex-col items-center justify-center border-b border-[#2C2523]/5 pb-4 w-full transition-colors text-center"
              >
          ...
          <div className="flex flex-col items-center text-center gap-4 border-t border-[#2C2523]/10 pt-6">
